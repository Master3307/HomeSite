...
const ACTIVITY_POLL_INTERVAL_MS = Math.max(
  15000,
...
if (!DISCORD_CLIENT_SECRET) throw new Error("Missing DISCORD_CLIENT_SECRET");
if (!DISCORD_REDIRECT_URI) throw new Error("Missing DISCORD_REDIRECT_URI");
...
async function persistActivityStore() {
  const rows = [...activityStore.values()].sort(
    (a, b) =>
